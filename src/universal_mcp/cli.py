...
@codegen_app.command()
def split_api(
    input_app_file: Path = typer.Argument(
        ..., help="Path to the single, generated Python API client file (e.g., app.py) to be split."
    ),
    output_dir: Path = typer.Option(
        ..., "--output-dir", "-o", help="Directory where the split Python files will be saved."
    ),
    package_name: str = typer.Option(
        None,
        "--package-name",
        "-p",
        help="Optional base package name for generating absolute imports in the split files (e.g., 'my_mcp_app.hubspot').",
    ),
):
...
@app.command()
def install(
    app_name: str = typer.Argument(..., help="Name of the desktop application to configure (e.g., 'claude', 'cursor')"),
):
...
@app.command()
def client(model: str = typer.Option(default="gpt-4o", help="LLM model to use")):
    """Start the AI Agent CLI"""
    import asyncio

    from universal_mcp.client.cli import AgentCLI
    from universal_mcp.logger import setup_logger

    setup_logger(level="WARNING")
    app = AgentCLI()

    asyncio.run(app.run(model))
