...
def graph_builder(llm, registry, instructions, model, executor_agent_cls):
    """Build the graph for the planner agent."""
    graph_builder = StateGraph(PlannerState)

    async def _tool_finder_node(state: PlannerState) -> dict[str, Any]:
    ...
    async def _executor_node(state: PlannerState) -> dict[str, Any]:
    ...
    async def _no_tools_node(state: PlannerState) -> dict[str, Any]:
