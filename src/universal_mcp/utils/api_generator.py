    ...
    try:
        ...
        try:
            with open(temp_output_path) as f:
                file_content = f.read()
                echo(
                    f"Successfully wrote {len(file_content)} bytes to {temp_output_path}"
                )
            ...
            async def run_docstring():
                ...
                try:
                    with open(script_path) as f:
                        content = f.read()
                        echo(
                            f"Successfully read {len(content)} bytes from {script_path}"
                        )
                except Exception as e:
        ...
        try:
            ...
            if class_obj and hasattr(class_obj, "list_tools"):
                try:
                    instance = class_obj()
                    tool_list_func = instance.list_tools
                    ...
                    for tool in tool_list:
                        ...
                        doc = (
                            tool.__doc__
                            or f"Function for {func_name.replace('_', ' ')}"
                        )
                        summary = doc.split("\n\n")[0].strip()
